...
public class ImageConverter {
    ...
    private static Integer getMaxPixelsFromFormIfDefined(QuestionWidget questionWidget) {
        ...
        for (TreeElement attrs : questionWidget.getFormEntryPrompt().getBindAttributes()) {
            if ("max-pixels".equals(attrs.getName()) && ApplicationConstants.XML_OPENROSA_NAMESPACE.equals(attrs.getNamespace())) {
                try {
    ...
    private static Integer getMaxPixelsFromSettings(Context context) {
        Integer maxPixels = null;
        String imageSizeMode = (String) GeneralSharedPreferences.getInstance().get(PreferenceKeys.KEY_IMAGE_SIZE);
